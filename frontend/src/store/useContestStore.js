...
export const useContestStore = create((set) => ({
  ...
  createContest: async (contestData) => {
    ...
    contestLeaderBoard: async (contestId) => {
        try {
            set({ isContestLoading: true });
            const res = await axiosInstance.post(`/contest/contest/${contestId}/leaderboard`);
    ...
    addProblemToContest: async (contestId, problemData) => {
        try {
            set({ isContestLoading: true });
            const res = await axiosInstance.post(`/contest/add-problem-to-contest/${contestId}`, problemData);
            toast.success(res.data.message || "Problem added to contest successfully");
    ...
    contestProblemSubmission: async (contestId, problemId, submissionData) => {
        try {
            set({ isContestLoading: true });
            const res = await axiosInstance.post(`/contest/contest-submission/${contestId}/${problemId}`, submissionData);
    ...
    getAllProblemsInContest: async (contestId) => {
        try {
            set({ isContestLoading: true });
            const res = await axiosInstance.get(`/contest/get-all-problems-in-contest/${contestId}`);
            ...
            console.error("Error occurred in fetching all problems in contest", error);
    ...
    getContestProblemsSolvedByUser: async () => {
        try {
            const res = await axiosInstance.get("/contest/getAllProblemsSolvedByUser");
            console.log(res);
            set({ solvedProblems: res.data.problemSolvedByUser });
            // toast.success(res.data.message);
        } catch (error) {
            console.error("Error occurred in fetching problems solved by user", error);
            toast.error("Error in fetching problems solved by user");
        }
    },
    updateContest: async (id, updatedData) => {
        try {
            const res = await axiosInstance.put(`/contest/update-contest/${id}`, updatedData);
            console.log(res);
            set((state) => ({
                contests: state.contests.map((contest) =>
                    contest._id === id ? { ...contest, ...res.data.updatedContest } : contest
                ),
            }));
    ...
    updateContestProblem: async (contestId, problemId, updatedData) => {
        try {
            const res = await axiosInstance.put(`/contest/update-problem/${contestId}/${problemId}`, updatedData);
            console.log(res);
    ...
    deleteContestProblem: async (contestId, problemId) => {
        try {
            const res = await axiosInstance.delete(`/contest/delete-problem/${contestId}/${problemId}`);
            console.log(res);
